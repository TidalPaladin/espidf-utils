/**
 * Library designed to provide easy access to non-volatile storage (NVS).
 * Data that should persist between reboots, such as configuration settings,
...
class NVSStatic {
 ...
 public:
   ...
   */
  template <typename T>
  static esp_err_t read(const char *key, T &dest) {
   ...
   */
  template <typename T>
  static esp_err_t write(const char *key, T &src) {
